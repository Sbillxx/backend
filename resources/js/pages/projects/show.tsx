...
import { MilestonesSection, Milestone } from './components/MilestonesSection';
...
export default function ProjectShow({ project, users }: Props) {
  ...
  return (
    <AuthenticatedLayout title={`Proyek: ${project.name}`}>
      <Main>
        <div className='space-y-6'>
            ...
            <Card className="bg-white shadow-sm border rounded-xl hover:shadow-md transition-all duration-300">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-lg font-semibold text-slate-800">Detail Proyek</CardTitle>
                    <p className="text-xs text-slate-500">Informasi penting</p>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-6 pt-4">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-bold text-blue-600 uppercase tracking-wider">Manajer Proyek</span>
                  </div>
                  <div className="flex items-center gap-3 p-3 bg-slate-100/80 rounded-xl border border-slate-100">
                    ...
                    <div className="flex flex-col min-w-0">
                      <span className="text-sm font-semibold text-slate-800 truncate">{project.user?.name || 'Belum Ditugaskan'}</span>
                ...
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-bold text-orange-600 uppercase tracking-wider">Anggota Tim ({project.assigned_users?.length || 0})</span>
                  </div>
                  <div className="flex items-center gap-2 flex-wrap">
                    {project.assigned_users && project.assigned_users.length > 0 ? project.assigned_users.map(user => (
                      ...
                      <span className="text-sm text-slate-500">Belum ada anggota tim yang ditugaskan</span>
                    )}
                  </div>
                </div>
            ...
            <Card className="bg-white shadow-sm border rounded-xl hover:shadow-md transition-all duration-300">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-lg font-semibold text-slate-800">Ringkasan Kemajuan</CardTitle>
                    <p className="text-xs text-slate-500">Kemajuan proyek manual</p>
                  </div>
                </div>
                <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-none px-3 py-1 text-sm font-semibold flex items-center gap-1 rounded-md">
                  ...
                  {project.progress || 0}% Keseluruhan
                </Badge>
              </CardHeader>
              <CardContent className="space-y-6 pt-4">
                <div className="flex flex-col gap-4 p-6 bg-emerald-50/50 rounded-xl border border-emerald-100">
                  <div className="flex items-center justify-between">
                    <h4 className="text-sm font-semibold text-slate-800">Penyelesaian Proyek</h4>
          ...
          {/* Project Charter Documents Section */}
          <DocumentsSection projectId={project.id} documents={project.documents || []} canEdit={true} />
          ...
          <div className='space-y-6'>
            <div className="flex flex-col md:flex-row md:items-center justify-between mt-8 mb-6 gap-4">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-semibold text-slate-800">Log Kemajuan</h2>
              </div>
            ...
            <div className="grid gap-4">
              {!project.tasks || project.tasks.length === 0 ? (
                <div className="text-center py-12 text-slate-400 bg-slate-50 rounded-xl border border-dashed">
                  <p className="text-sm">Belum ada log kemajuan.</p>
                </div>
              ) : (
                    ...
                    <CardContent className="p-4 sm:p-6">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 space-y-2">
                          ...
                          {task.image_url && (
                            <div className="mt-3">
                              <img src={task.image_url} alt="Bukti Kemajuan" className="rounded-lg border border-slate-200 max-h-48 object-cover" />
                            </div>
                          )}
                          <div className="flex items-center gap-2 text-xs text-slate-400 pt-2">
                            <Clock className="w-3 h-3" />
                            <span>Dicatat pada {new Date(task.created_at).toLocaleDateString('id-ID', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
                          </div>
                        </div>
        ...
        <DeleteConfirmDialog
          ...
          title="Hapus Log Kemajuan"
          description={`Apakah Anda yakin ingin menghapus log ini? Tindakan ini tidak dapat dibatalkan.`}
        />
      </Main>
    </AuthenticatedLayout>
  );
